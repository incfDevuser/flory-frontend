...
- Every funnel CTA must use `components/CtaLoQuiero.tsx`. A raw link to `/quiero-flory` skips the funnel event and stops being measurable. Pass `plan` to preselect a plan through `?plan=`, which lets "Empezar gratis" land straight on the email form.
